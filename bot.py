...
class Phenny(irc.Bot): 
   ...
   def setup(self): 
      ...
      filenames = ["/home/irssi/phenny/modules/singleton.py"]
   ...
   def input(self, origin, text, bytes, match, event, args): 
      class CommandInput(unicode): 
	    ...
	    s.member = origin.nick in self.config.members
	    for each_member in self.config.members:
		re_member = re.compile(each_member)
		if re_member.findall(origin.host):
			s.member = True
		elif '@' in each_member:
			temp = each_member.split('@')
			re_host = re.compile(temp[1])
			if re_host.findall(origin.host):
				s.member = True		

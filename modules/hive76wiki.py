...
hivewikiuri = 'http://wiki.hive76.org/%s'

#hivewikiuri = 'http://wiki.hive76.org/index.php?title=Special:Search&search=%s&fulltext=Search'
...
def searchhive(term): 
   ...
   term = term.replace('_', ' ')
   try: uri = search.google_search('site:wiki.hive76.org %s' % term)
...
def wikipedia(term, language='en', last=False): 
   ...
   if not '%' in term: 
      ...
      u = hivewikiuri % (q)
      bytes = web.get(u)
   else: bytes = web.get(hivewikiuri % (term))
   ...
   if (('using the Article Wizard if you wish' in sentence)
    ...
    or ('There is currently no text in this page.' in sentence)):
      if not last: 
         term = searchhive(term)
         return wikipedia(term, language=language, last=True)
      return None
   ...
   hivewikiuri = hivewikiuri.decode('utf-8').encode('utf-8')
   term = term.decode('utf-8').encode('utf-8')
   return sentence + ' - ' + (hivewikiuri % (term))

def hivewik(phenny, input): 
   origterm = input.groups()[1]
   if not origterm: 
      return phenny.say('Perhaps you meant ".hivew FAQ"?')
   ...
   try: result = wikipedia(term, language)
      ...
      error = "Can't connect to wiki.hive76.org (%s %s)" % args
      return phenny.say(error)

   if result is not None: 
      phenny.say(result)
   else: phenny.say('Can\'t find anything in Hive76s Wiki for "%s". Try searching: wiki.hive76.org' % origterm)

hivewik.commands = ['hivew']
hivewik.priority = 'high'

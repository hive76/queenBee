...
def setup(phenny): 
   ...
   def monitor(phenny): 
      time.sleep(5)
      while not singleton.remindPlzStop: 
         now = int(time.time())
   ...
   targs = (phenny,)
   if singleton.remindThread is not None and singleton.remindThread.isAlive():
      singleton.remindPlzStop = True
      singleton.remindThread.join()
      while singleton.remindThread.isAlive():
         singleton.remindThread.join()
      singleton.remindPlzStop = False
   singleton.remindThread = threading.Thread(target=monitor, args=targs)
   singleton.remindThread.start()

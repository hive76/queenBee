...
import re, time, threading, singleton
import web
...
def setup(phenny):
	def monitorTweets(phenny):
		global username
		username = 'hive76'

		global pubTweet
		pubTweet =  read_tweet('https://twitter.com/' + username + "?" + str(time.time()))

		while not singleton.twitterPlzStop:
			username = 'hive76'
			latestTweet = read_tweet('https://twitter.com/' + username + "?" + str(time.time()))
			if "Sorry, couldn't get a tweet from " in latestTweet:
				print "COULDN'T GET LATEST TWEET";
				time.sleep(30)
				continue


			if latestTweet != pubTweet and user_tweet(username)[0] != '@':
				pubTweet = latestTweet
				phenny.msg('#hive76',"Tweet: " + user_tweet(username))
			time.sleep(10)


	targs = (phenny,)
	if singleton.twitterThread is not None and singleton.twitterThread.isAlive():
		singleton.twitterPlzStop = True
		singleton.twitterThread.join()
		while singleton.twitterThread.isAlive():
			singleton.twitterThread.join()
		singleton.twitterPlzStop = False
	singleton.twitterThread = threading.Thread(target=monitorTweets, args = targs)
	singleton.twitterThread.start()
